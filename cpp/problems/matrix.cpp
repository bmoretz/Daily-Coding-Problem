...
namespace problems::matrix
{
	...
	std::vector<int> matrix::operator[]( const int row ) const
	{
		_ASSERT( row >= 0 && row <= height() );
	...
	std::vector<int>& matrix::operator[]( const int row )
	{
		_ASSERT( row >= 0 && row <= height() );
	...
	{
		auto vectors = std::vector<std::vector<int>>( m.height() );
		...
		for( auto row = 0; row < m.height(); ++row )
		{
			...
			for( auto index = 0; index < values.size(); ++index )
			{
				if( vectors[ index ].empty() )
					vectors[ index ] = std::vector<int>( m.width() );
	...
	{
		auto zero_rows = std::vector<int>(), zero_columns = std::vector<int>();
